...
from models import *
# forms and validation to accept the input from the user
...
class BookingForm(FlaskForm):
    h_name = StringField('Hotel', validators=[
                         DataRequired(), Length(min=2, max=20)])
    cr_name = StringField('Car Rental', validators=[
                          DataRequired(), Length(min=2, max=20)])
    b_start_date = DateField('From Date', validators=[
                             DataRequired()], format="%Y-%m-%d")
    b_end_date = DateField('To Date', validators=[
                           DataRequired()], format="%Y-%m-%d")
